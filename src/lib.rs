...
pub async fn run() {
    ...
    let context = Arc::new(ModelLoadContext {
        device: Arc::new(ThreadSafeDevice::new(device.clone())),
        queue: Arc::clone(&queue),
        fetched_images: Arc::new(parking_lot::Mutex::new(fetched_images)),
    ...
    let models = Arc::new(parking_lot::Mutex::new(models));

    log::info!("urls: {:?}", model_urls);

    for (i, (_, url)) in preloaded_model_data.into_iter().enumerate() {
        {
            let models = Arc::clone(&models);
            let context2 = Arc::clone(&context);
            let context = context.clone();
            context2.thread_pool.spawn_lazy(move || {
                Box::pin(async move {
                    use futures::AsyncReadExt;

                    let mut async_reader = async_reader_from_fetch(&url, None).await.unwrap();

                    let mut bytes = Vec::new();
                    async_reader.read_to_end(&mut bytes).await.unwrap();
                    let model = load_gltf_from_bytes(&bytes, Some(url), context).await;
                    models.lock()[i] = model.unwrap();
                })
            });
        }
    ...
    wasm_webxr_helpers::Session { inner: xr_session }.run_rendering_loop(move |_time, frame| {
        let models = match models.try_lock() {
            Some(models) => models,
            None => return,
        };
        ...
        let player_states = match player_states.try_lock() {
            Some(player_states) => player_states,
            None => return,
        };
        ...
        let player_hands: Vec<Instance> = std::iter::once(player_state.clone())
            .chain(player_states.values().cloned())
        ...
        let opaque_model_primitives: Vec<Vec<_>> = models
            ...
            .map(|model| {
                model
                    .opaque_primitives
                    .iter()
                    .map(|primitive| primitive.bind_group.try_lock())
                    .collect::<Vec<_>>()
            })
            .collect();
...
fn render_primitives<'a>(
    ...
    bind_groups: &'a [Option<parking_lot::MutexGuard<wgpu::BindGroup>>],
    viewports: &[Viewport],
    ...
    for (primitive_index, primitive) in primitives.iter().enumerate() {
        ...
        if let Some(bind_group) = &bind_groups[primitive_index] {
